...
mkdir -p ./volumes/qbittorrent/downloads
`;
  };

  const checkVolumesDirectory = () => {
    return `
if [[ ! -d ./volumes/qbittorrent/config ]]; then
  echo "qBittorrent config directory is missing!"
  sleep 2
fi
if [[ ! -d ./volumes/qbittorrent/downloads ]]; then
  echo "qBittorrent downloads directory is missing!"
  sleep 2
fi
`;
  };
