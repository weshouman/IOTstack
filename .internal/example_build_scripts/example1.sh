...
# Generate build
CBRES=$(curl -s \
  -d '{"buildOptions":{"selectedServices":["mosquitto","nodered","influxdb","grafana"],"serviceConfigurations":{"services":{"nodered":{"devices":["/dev/ttyAMA0:/dev/ttyAMA0","/dev/vcio:/dev/vcio","/dev/gpiomem:/dev/gpiomem"],"loggingEnabled":true,"networks":{"iotstack_nw":true},"networkMode":"unchanged"}},"networks":{},"meta":{}}}}' \
