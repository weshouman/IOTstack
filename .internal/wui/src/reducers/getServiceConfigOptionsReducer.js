...
import { GET_CONFIG_SERVICE_CONFIG_OPTIONS } from '../actions/getServiceConfigOptions.action';
...
const reducerHandler = (state = defaultState, action) => {
  ...
  switch (action.type) {
    case `${GET_CONFIG_SERVICE_CONFIG_OPTIONS}_${API_STATUS.PENDING}`:
      if (state.services.pending.indexOf(action.label) > -1) {
        console.warn(`getServiceConfigOptionsReducer: '${action.label}' already dispatched`, action);
      }
    ...
    case `${GET_CONFIG_SERVICE_CONFIG_OPTIONS}_${API_STATUS.SUCCESS}`:
    ...
    case `${GET_CONFIG_SERVICE_CONFIG_OPTIONS}_${API_STATUS.FAILURE}`:
...
export const getConfigServiceConfigOptionsSelector = (state) => {
  return state.configServiceConfigOptionsSelector
};
